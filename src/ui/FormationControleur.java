...
public class FormationControleur {
    ...
    if (ges.getNomFormation() != null) {
      ...
      switch (resultat) {
        case 0:
          break;
        case -1:
          this.afficherPopup("Le déplacement de TD n'a pas pu être fait.",
              AlertType.ERROR);
          break;
        case -2:
          this.afficherPopup("Le déplacement de TP n'a pas pu être fait.",
              AlertType.ERROR);
          break;
        case -3:
          this.afficherPopup(
              "Les déplacements de TD et de TP n'ont pas pu être faits.",
              AlertType.ERROR);
          break;
        default:
  ...
  void actionBoutonAfficherEtudiantsGroupeTD(ActionEvent event) {
    if (ges.getNomFormation() != null) {
      if (entreeGroupeTDEtudiant.getText() != null
          && !entreeGroupeTDEtudiant.getText().isEmpty()) {
        try {
  ...
  void actionBoutonAfficherEtudiantsGroupeTP(ActionEvent event) {
    if (ges.getNomFormation() != null) {
      if (entreeGroupeTPEtudiant.getText() != null
          && !entreeGroupeTPEtudiant.getText().isEmpty()) {
        try {
    ...
    if (ges.getNomFormation() != null) {
      try {
        ...
        if (observableEtudiants.size() != 0) {
          ...
          this.afficherPopup("Aucun Étudiants est inscrit à l'UE",
              AlertType.ERROR);
        }
  ...
  void actionBoutonAfficherTousEtudiants(ActionEvent event) {
    if (ges.getNomFormation() != null) {
      if (ges.getGestionEtudiant().getListeEtudiants().size() != 0) {
        ...
        this.afficherPopup("Aucun Étudiants dans la formation",
            AlertType.ERROR);
      }
    } else {
      this.afficherPopup("Aucune Formation", AlertType.ERROR);
  ...
  void actionBoutonCreerFormation(ActionEvent event) {
    ges.creerFormation(entreeNomFormation.getText(),
    ...
    if (ges.getNomFormation() != null) {
      ...
      if (entreeNomResponsableFormation.getText() == null
          || entreeEmailResponsableFormation.getText().isEmpty()) {
        this.afficherPopup("Les champs nécessaire ne sont pas tous remplis !",
            AlertType.ERROR);
      } else {
  ...
  void actionBoutonNombreChoixOptions(ActionEvent event) {
    if (ges.getNomFormation() != null) {
      if (entreeNombreChoixOptions.getText() != null
          && !entreeNombreChoixOptions.getText().isEmpty()) {
        try {
          ges.definirNombreOptions(
              Integer.parseInt(entreeNombreChoixOptions.getText()));
          ges.getGestionEtudiant().getListeEtudiants()
              .forEach(etudiant -> ges.setNbOptionEtudiant(etudiant));
        } catch (NumberFormatException e) {
  ...
  void actionBoutonSetTailleGroupeTD(ActionEvent event) {
    if (ges.getNomFormation() != null) {
      if (entreeTailleGroupeTD.getText() != null
          && !entreeTailleGroupeTD.getText().isEmpty()) {
        try {
          ges.setTailleGroupeDirige(
              Integer.parseInt(entreeTailleGroupeTD.getText()));
        } catch (NumberFormatException e) {
          this.afficherPopup("Le champ est incorecte !", AlertType.ERROR);
        }
      } else {
   ...
   * champ de saisie de la taille du groupe TD n'est pas vide et s'il contient
   * une valeur numérique valide. En cas de succès, met à jour la taille du
   * groupe dirigé de la formation. En cas d'échec, affiche une boîte de
   * dialogue d'erreur appropriée.
   *
   * @param event L'événement déclenché par le clic sur le bouton.
   */
  @FXML
  void actionBoutonSetTailleGroupeTP(ActionEvent event) {
    if (ges.getNomFormation() != null) {
      if (entreeTailleGroupeTP.getText() != null
          && !entreeTailleGroupeTP.getText().isEmpty()) {
        try {
          ges.setTailleGroupePratique(
              Integer.parseInt(entreeTailleGroupeTP.getText()));
        } catch (NumberFormatException e) {
          this.afficherPopup("Le champ est incorecte !", AlertType.ERROR);
        }
      } else {
    ...
    try {
      ...
      if (ges.nombreGroupesTravauxDiriges() != -1) {
        entreeTailleGroupeTD
            .setText(Integer.toString(ges.getTailleGroupeDirige()));
      }
      if (ges.nombreGroupesTravauxDiriges() != -1) {
        entreeTailleGroupeTP
            .setText(Integer.toString(ges.getTailleGroupePratique()));
      }
      if (ges.nombreGroupesTravauxDiriges() != -1) {
        entreeNombreChoixOptions.setText(Integer.toString(ges.getNBoption()));
      }
      ges.getGestionEtudiant().getListeUE().stream()
          .filter(ue -> ue.getOptionnel())
          .forEach(ue -> listeUEOptionnelles.getItems().add(ue.getNomUE()));
      ges.getGestionEtudiant().getListeUE().stream()
          .filter(ue -> !ue.getOptionnel())
          .forEach(ue -> listeUEObligatoires.getItems().add(ue.getNomUE()));
      ges.getGestionEtudiant().getListeEtudiants().stream()
          .forEach(etu -> listeEtudiants.getItems()
              .add(Integer.toString(etu.getNumero())));
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
  ...
  void actionBoutonCreerNouvelleUE(ActionEvent event) {
    if (ges.getNomFormation() != null) {
      if (entreeNomUE.getText() != null && !entreeNomUE.getText().isEmpty()
          && entreeNomResponsableUE.getText() != null
          && !entreeNomResponsableUE.getText().isEmpty()) {
        UniteEnseignement ue = new UniteEnseignement(entreeNomUE.getText(),
            entreeNomResponsableUE.getText());
        try {
          ...
          this.afficherPopup("La capacité d'accueil doit être un entier",
              AlertType.ERROR);
        }
        entreeNomResponsableUE.setText("");
        entreeCapaciteAccueil.setText("");
