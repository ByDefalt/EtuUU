...
public class EtudiantsControleur {
  ...
  /**
   * Gère l'action associée au bouton "Choisir Option". Cette méthode est
   * appelée lorsqu'un utilisateur clique sur le bouton pour choisir une option
   * parmi les Unités d'Enseignement optionnelles de la formation. Si
   * l'utilisateur est connecté, il peut choisir une option, et la liste des
   * Unités d'Enseignement suivies par l'étudiant est mise à jour.
   *
  ...
  void actionBoutonChoisirOption(ActionEvent event) {
    try {
      ...
      if (ueNom != null) {
        for (UniteEnseignement ue : this.gestionFormation.getGestionEtudiant()
            .getListeUE()) {
      ...
      ObservableList<String> listeUe =
          FXCollections.observableArrayList(this.gestionFormation
              .getGestionEtudiant().enseignementsSuivis().stream()
              .map(UniteEnseignement::getNomUE).collect(Collectors.toSet()));
  ...
  /**
   * Gère l'action associée au bouton "Connexion". Cette méthode est appelée
  ...
  void actionBoutonConnexion(ActionEvent event) {
    if (entreeNumeroEtudiant.getText().isEmpty()
        || entreeMotDePasseEtudiant.getText().isEmpty()) {
      this.afficherPopup(
          "Les champs nécessaire a la connexion ne sont pas tous remplis !",
          AlertType.ERROR);
    } else {
      try {
        ...
        if (res) {
          Etudiant etudiant =
              this.gestionFormation.getGestionEtudiant().getEtudiantConnecte();
          entreeNomEtudiant
              .setText(etudiant.getInformationPersonnelle().getNom());
          entreePrenomEtudiant
              .setText(etudiant.getInformationPersonnelle().getPrenom());
          entreeAdresseEtudiant
              .setText(etudiant.getInformationPersonnelle().getAdresse());
          entreeAgeEtudiant.setText(
              String.valueOf(etudiant.getInformationPersonnelle().getAge()));
          ...
          if (this.gestionFormation.getGestionEtudiant()
              .inscriptionFinalisee()) {
            checkInscriptionFinalisee.setSelected(true);
          } else {
            checkInscriptionFinalisee.setSelected(false);
          }
          ObservableList<String> listeUe =
              FXCollections.observableArrayList(etudiant.getListeUEsuivies()
                  .stream().map(UniteEnseignement::getNomUE)
                  .collect(Collectors.toSet()));
          listeUESuiviesEtudiant.setItems(listeUe);
          
          ObservableList<String> listeUEOptionnelles =
              FXCollections.observableArrayList(this.gestionFormation
                  .getGestionEtudiant().enseignementsOptionnels().stream()
                  .map(UniteEnseignement::getNomUE)
                  .collect(Collectors.toSet()));
          listeUEOptionnellesFormation.setItems(listeUEOptionnelles);
          
          ObservableList<String> messages = FXCollections.observableArrayList(
              this.gestionFormation.getGestionEtudiant().listeTousMessages());
          listeTousMessages.setItems(messages);
          
          ObservableList<String> messagesNonLus =
              FXCollections.observableArrayList(this.gestionFormation
                  .getGestionEtudiant().listeMessageNonLus());
          listeMessagesNonLus.setItems(messagesNonLus);
          map.clear();
          int i = 0;
          int j = 0;
          for (Message mes : this.gestionFormation.getGestionEtudiant()
              .getEtudiantConnecte().getMessages()) {
            if (!mes.estLu()) {
              map.put(i, j);
              i++;
            }
          ...
          this.afficherPopup("Le numéro ou le mot de passe est incorrect.",
              AlertType.ERROR);
        }
      } catch (NumberFormatException e) {
        this.afficherPopup("Le numéro étudiant doit être un nombre.",
            AlertType.ERROR);
      } catch (NonConnecteException e) {
    ...
    if (!entreeNomEtudiant.getText().isEmpty()
      ...
      try {
        InformationPersonnelle info =
            new InformationPersonnelle(entreeNomEtudiant.getText(),
                entreePrenomEtudiant.getText(), entreeAdresseEtudiant.getText(),
                Integer.parseInt(entreeAgeEtudiant.getText()));
        int resInscription = this.gestionFormation.getGestionEtudiant()
            .inscription(info, entreeMotDePasseEtudiant.getText());
        if (resInscription != -1) {
          /*
           * entreeNomEtudiant.setText(""); entreePrenomEtudiant.setText("");
           * entreeAdresseEtudiant.setText(""); entreeAgeEtudiant.setText("");
           */
      ...
      this.afficherPopup(
          "Les champs nécessaire a l'inscription ne sont pas tous remplis !",
          AlertType.ERROR);
    }
  }
  ...
  void actionBoutonRafraichirListesMessages(ActionEvent event) {
    try {
      ObservableList<String> messages = FXCollections.observableArrayList(
          this.gestionFormation.getGestionEtudiant().listeTousMessages());
      listeTousMessages.setItems(messages);
      ObservableList<String> messagesNonLus = FXCollections.observableArrayList(
          this.gestionFormation.getGestionEtudiant().listeMessageNonLus());
      listeMessagesNonLus.setItems(messagesNonLus);
  ...
  void actionSelectionMessageListeTousMessages(MouseEvent event) {
    try {
      String selectMessage =
          listeTousMessages.getSelectionModel().getSelectedItem();
      int selectIndex =
          listeTousMessages.getSelectionModel().getSelectedIndex();
      ...
      if (selectMessage != null) {
        ...
        zoneTexteContenuMessage.setText(message.getContenu());
        this.gestionFormation.getGestionEtudiant().getEtudiantConnecte()
            .getMessages().get(selectIndex).setLu();
      }
    } catch (NonConnecteException e) {
      this.afficherPopup("Vous n'êtes pas connecté", AlertType.ERROR);
    }
